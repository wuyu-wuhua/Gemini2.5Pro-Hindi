...
function getSafeTranslations() {
    ...
    return {
        ...
        hi: { 'untitled-chat': 'Без названия чата', 'history-title': 'История чата', 'no-history': 'Еще нет истории', 'welcome-message': 'Привет! Чем могу помочь?' }
    };
}
...
function switchLanguage(lang) {
    if (lang && lang === 'hi') { // Only hi is supported now
        localStorage.setItem('language', lang); // Set in storage first

        if (currentLangSpan) {
            currentLangSpan.textContent = 'Русский'; // Directly set to hi
        }
...
function applyTranslations(languageToApply) {
    const currentLang = languageToApply || localStorage.getItem('language') || 'hi';
    const trans = getSafeTranslations();
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    langOptions.forEach(option => {
        ...
        if (option.getAttribute('data-lang') !== 'hi') option.style.display = 'none'; else option.style.display = 'block';
    });
...
if (langOptions) {
    langOptions.forEach(option => {
        ...
        newOption.addEventListener('click', (e) => {
            ...
            if (lang === 'hi') { // Only allow switching to RU
                switchLanguage(lang);
            }
        });
    });
